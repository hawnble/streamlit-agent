...
if prompt := st.chat_input(placeholder="가볍고 빠른 노트북 추천해줄래? 무게는 1.5kg 이하면 좋아!"):
    ...
    if len(df_s) >= 1:
        ...
        pandas_df_agent = create_pandas_dataframe_agent(
            ...
            prefix = '너는 노트북을 전문적으로 추천해주는 챗봇 Pick-Chat!이야. 가격과 무게와 화면크기와 추천이유를 말해줘. 다른 정보는 요청시에만 제공해.한글로 답변을 작성해. 하이퍼링크와 외부주소를 작성하면 안되. Display_Point, Value_for_Money_Point, Value_Point 는 공개하지마.'
        )
