...
df = pd.read_csv("laptop_sdf_231014.csv")
#df = df.astype(str)
df.pop('Unnamed: 0')
#df
...
항상 가격과 무게와 화면크기를 제공해. 추천이유를 설명해줘. 다른 정보는 요청시에만 제공해.
...
if prompt := st.chat_input(placeholder="가볍고 빠른 노트북 추천해줄래? 무게는 1.5kg 이하면 괜찮을거 같아!"):
    ...
    llm = ChatOpenAI(
        temperature=0.5, model="gpt-4-0613", openai_api_key=openai_api_key, streaming=True
    )
